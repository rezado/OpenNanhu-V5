...
class Ftq(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper
  ...
  copied_last_cycle_bpu_in_for_ftq.zip(copied_last_cycle_bpu_in_ptr_for_ftq).zipWithIndex.map { case ((in, ptr), i) =>
    when(in) {
  ...
  val ftq_pd_mem = Module(new SyncDataModuleTemplate(new Ftq_pd_Entry, FtqSize, FtqRedirectAheadNum+1, 1, perReadPortBypassEnable = Some(Seq.fill(FtqRedirectAheadNum+1)(false)) , hasRen = true))
  ...
  def updateCfiInfo(redirect: Valid[Redirect], isBackend: Boolean = true) = {
    val r_valid   = redirect.valid
    val r_ptr     = redirect.bits.ftqIdx
    val r_idx     = redirect.bits.ftqIdx.value
    val r_offset  = redirect.bits.ftqOffset
    val r_taken   = redirect.bits.cfiUpdate.taken
    val r_mispred = redirect.bits.cfiUpdate.isMisPred
