...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule
{
  ...
  // valid only for the first gpfAdd commentMore actions
  ...
  if (env.EnableDifftest) {
    val discards = (0 until PortNumber).map { i =>
      ExceptionType.hasException(toIFU.bits.exception(i)) || toIFU.bits.pmp_mmio(i) || Pbmt.isUncache(toIFU.bits.itlb_pbmt(i))
    }
