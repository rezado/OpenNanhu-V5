...
class WbFuBusyTableImp(override val wrapper: WbFuBusyTable)(implicit  p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper) {
  ...
  // writeBusyTable(vfWbBusyTable, fpAllBusyTableWithParms, VfWB())
  ...
  // writeConflict(fpConflict, fpAllDeqRespSetWithParms, VfWB())
  ...
  // readRes(fpAllRespRead, vfWbBusyTable, VfWB())
  ...
  // readRes(fpAllWbConflictFlag, fpConflict, VfWB())
