...
class VfByteMaskTailGenIO(vlen: Int)(implicit p: Parameters) extends Bundle {
  ...
  val in = Input(new Bundle {
    ...
    val vta = Bool()
    val vsew = VSew()
    val maskUsed = UInt((numBytes / 2).W)
    val vdIdx = UInt(3.W)
    val isHi = Bool()
  })
  val out = Output(new Bundle {
    val activeEn   = UInt((numBytes / 2).W)
    val agnosticEn = UInt((numBytes / 2).W)
  })
...
class VfByteMaskTailGen(vlen: Int)(implicit p: Parameters) extends Module {
  require(isPow2(vlen))
  ...
  val io = IO(new VfByteMaskTailGenIO(vlen))
  ...
  private val prestartEnInVd = LookupTree(Cat(vdIdx, io.in.isHi), (0 until maxVLMUL*2).map(i => i.U -> prestartEn((i+1)*(numBytes/2) - 1, i*(numBytes/2))))
  private val bodyEnInVd = LookupTree(Cat(vdIdx, io.in.isHi), (0 until maxVLMUL*2).map(i => i.U -> bodyEn((i+1)*(numBytes/2) - 1, i*(numBytes/2))))
  private val tailEnInVd = LookupTree(Cat(vdIdx, io.in.isHi), (0 until maxVLMUL*2).map(i => i.U -> tailEn((i+1)*(numBytes/2) - 1, i*(numBytes/2))))
  ...
  private val activeEn = Mux(io.in.begin >= io.in.end, 0.U((numBytes/2).W), bodyEnInVd & maskEn)
  private val agnosticEn = Mux(io.in.begin >= io.in.end, 0.U((numBytes/2).W), maskAgnosticEn | tailAgnosticEn)
