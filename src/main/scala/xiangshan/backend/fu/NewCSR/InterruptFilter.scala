...
class InterruptFilter extends Module {
  ...
  val intrVec = Mux(disableAllIntr, 0.U, Mux(io.in.nmi, io.in.nmiVec, normalIntrVec))
  ...
  io.out.interruptVec.valid := delayedIntrVec.orR || delayedVIIsHvictlInjectReg || delayedDebugIntr
