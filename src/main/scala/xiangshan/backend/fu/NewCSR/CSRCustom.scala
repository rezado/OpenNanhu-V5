...
class SmblockctlBundle extends CSRBundle {
  val REPLAY_ESCAPE_TIME               = ReplayEscapeValue(14,11).withReset(ReplayEscapeValue.initValue)
  val ENABLE_REPLAY_ESCAPE             = RW(  10).withReset(true.B)
...
object StuckValue extends CSREnum with RWApply {
  val initValue = Value(16.U)
}

object ReplayEscapeValue extends CSREnum with RWApply {
