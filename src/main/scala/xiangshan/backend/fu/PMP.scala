  ...
  with PMACheckMethod
{
  ...
  val pmp_cmd = if (leaveHitMux) RegEnable(req.cmd, io.req.valid) else req.cmd
  val resp_pmp = pmp_check(pmp_cmd, res_pmp.cfg)
  val resp_pma = pma_check(pmp_cmd, res_pma.cfg)
