...
class BusyTable(numReadPorts: Int, numWritePorts: Int, numPhyPregs: Int, pregWB: PregWB)(implicit p: Parameters, params: SchdBlockParams) extends XSModule with HasPerfEvents {
  ...
  val perfEvents = Seq(
    (s"${btName}bt_std_freelist_1_4_valid", busyCount < (numPhyPregs / 4).U                                      ),
    (s"${btName}bt_std_freelist_2_4_valid", busyCount > (numPhyPregs / 4).U && busyCount <= (numPhyPregs / 2).U    ),
    (s"${btName}bt_std_freelist_3_4_valid", busyCount > (numPhyPregs / 2).U && busyCount <= (numPhyPregs * 3 / 4).U),
    (s"${btName}bt_std_freelist_4_4_valid", busyCount > (numPhyPregs * 3 / 4).U)
  )
  generatePerfEvent()
}
