...
class TraceBuffer(implicit val p: Parameters) extends Module
  ...
  val traceTrap    = Reg(Valid(new TraceTrap))
  val traceEntries = Reg(Vec(CommitWidth, Valid(new TraceBlock(false, IretireWidthCompressed))))
  ...
  val iretireSum = Wire(Vec(CommitWidth, UInt(IretireWidthInPipe.W)))
  for(i <- 0 until CommitWidth) {
    if (i == 0) {
        iretireSum(i) := io.in.fromRob.blocks(i).bits.tracePipe.iretire
    } else {
        when(!inTypeIsNotNoneVec(i)){
            iretireSum(i) := iretireSum(i - 1) + io.in.fromRob.blocks(i).bits.tracePipe.iretire
        }.otherwise{
            iretireSum(i) := io.in.fromRob.blocks(i).bits.tracePipe.iretire
        }
    }
