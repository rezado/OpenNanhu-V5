...
class PTW()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  val stage1Hit = RegEnable(io.req.bits.stage1Hit, false.B, io.req.fire)
  val stage1 = RegEnable(io.req.bits.stage1, io.req.fire)
  val hptw_resp_stage2 = RegInit(false.B)
  ...
  ptw_resp.apply(resp_pf, resp_af, resp_level, resp_pte, vpn, satp.asid, hgatp.vmid, vpn(sectortlbwidth - 1, 0), not_super = false, not_merge = false)
...
//  val dup_vec_addr_check = dup_vec.zip(state).map{ case(d,s) => d && (s === state_addr_check) }
  ...
  val to_mem_out = dup_wait_resp && noStage2
